...
class RelPositionalEncoding(torch.nn.Module):
    ...
    def forward(
        ...
        if offset > 0:
            ...
            # This branch handels x_T == 0.
            # In theory, this branch should be included in
            # previous branch "elif x_T > 1";
            # As for x_T == 0 what expected is:
            # pos_emb = torch.cat([[], center of self.pe, []], dim=1)
            #
            # However, for pos_emb.shape == [1, 1, C],
            # pos_emb[:, :0] = tensor([])
            # While pos_emb[:, -0:] is with shape [1, C]
            # instead of being the expected tensor([])
            # which results a wrong pos_emb
            # So, x_T == 0 is specially handled here.
            assert x_T == 1
            assert pos_emb.size(1) == 1
