...
class CodebookIndexExtractor:
    ...
    def init_dirs(self):
        ...
        self.dst_dir = Path("./data/vq_fbank")
    ...
    def merge_vq_manifests(self):
        """
        Merge generated vq included manfiests and storage to self.dst_dir.
        """
        for subset in self.params.subsets:
            vq_manifests = f"{self.manifest_dir}/with_codebook_indexes-cuts_train-{subset}*.json.gz"
            dst_vq_manifest = self.dst_dir / f"cuts_train-{subset}.json.gz"
    ...
    def extract_codebook_indexes(self):
        if self.params.world_size == 1:
            ...
            # release the GPU memory occopied by current extractor.
...
def compute_codebook_indexes_parallel(
    ...
    """Create an extractor for each rank and extract codebook indexes parallely.
